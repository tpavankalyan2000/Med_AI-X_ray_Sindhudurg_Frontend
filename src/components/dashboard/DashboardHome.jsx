...
const DashboardHome = () => {
  const { currentUser } = useAuth();
    ...
    useEffect(() => {
      const fetchStats = async () => {
        try{
          const response = await fetch(`${URLS.API_URL}/stats_dashboard`);
          const data = await response.json();
          setDashStats(data.results);
        } catch (error){
  ...
  // ];
    const iconMap = {
      // chat_sessions: FiMessageSquare,
      xray_analyses: FiFileText,
      // extracted_images: FiUsers,
      documents_analysed: FiCalendar,
    };
    const colorMap = {
      // chat_sessions: "bg-blue-500",
      xray_analyses: "bg-green-500",
      // extracted_images: "bg-purple-500",
      documents_analysed: "bg-amber-500",
    };
    const labels = {
      // chat_sessions: "AI Chat Sessions",
      xray_analyses: "X-Ray Analyses",
      // extracted_images: "Extracted X-Ray Images",
      documents_analysed: "Documents Analysed",
    };
    const stats = Object.entries(dashStats).map(([key, value], index) => ({
      id: index + 1,
      name: labels[key] || key,
      value: value,
      icon: iconMap[key],
